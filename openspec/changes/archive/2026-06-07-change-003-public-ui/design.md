...
> **Scope note**: This design documents the END state of the public UI. The implementation is split between this change (`change-003-public-ui`, **UI shell only**) and `change-004-cart-whatsapp` (transactional layer). Sections 5-7 (Pydantic schemas, route handlers, sequence diagrams for the order POST, WhatsApp formatter) describe code that is implemented in `change-004-cart-whatsapp`, not here. This change implements: section 1 (overview), section 2 (file tree for UI shell only), section 3 (UI shell modules), section 4 (CSS architecture), section 5 (JS architecture — `cart.js` only, `order.js` is change-004), section 6 (mascot SVG, placeholder SVGs), section 7 (template hierarchy for the home page only), section 12 (decisions scoped to the UI shell), section 13 (out of scope — checkout is change-004), section 14 (risks for the UI shell), section 15 (smoke test for the UI shell), section 16 (dependencies).
