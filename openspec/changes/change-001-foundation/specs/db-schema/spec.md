...
> **Note**: the proposal mentions 3 tables, but during design we added a `productos` table (with a `disponible` toggle column) because product data is structured and needs admin-controlled Sold Out state, which does not fit the `configuracion` key/value shape.
